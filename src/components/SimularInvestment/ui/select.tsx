...
const SelectTrigger = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger> & {
    multiplo30: boolean;
    selectedValue?: number; // Add this line to accept the selected value
  }
>(({ className, children, multiplo30, selectedValue, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'absolute right-8 flex h-10 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:bg-slate-950 dark:ring-offset-slate-950 dark:placeholder:text-slate-400 dark:focus:ring-slate-300 [&>span]:line-clamp-1',
      className,
    )}
    {...props}
  >
